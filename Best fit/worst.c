...
int main() {
    ...
    // Finding the worst-fit block to allocate each memory
    for(int i = 0; i < num_memory; i++) {
        int worstIndex = -1;
        for(int j = 0; j < num_blocks; j++) {
            if(b[j].allocated != 1 && b[j].size >= memory[i].size) {
                if(worstIndex == -1  || b[j].size > b[worstIndex].size) {
                    worstIndex = j;
                }
            }
        }
        if(worstIndex != -1) {
            // Allocate memory
            b[worstIndex].allocated = 1;
            memory[i].blockNo = b[worstIndex].blockNo;
            memory[i].fragment = b[worstIndex].size - memory[i].size;
            memory[i].blockSize = b[worstIndex].size;
        }
        else{
            memory[i].blockNo = 0;
            memory[i].fragment = 0;
